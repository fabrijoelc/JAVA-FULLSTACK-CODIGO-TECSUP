...
public class ProductoControllerTest {
    ...
    public void testObtenerProducto() {
        Producto producto = Producto.builder().id(1L).nombre("Laptop").precio(3000.0).categoria("Tecnologia").build();

        when(authValidator.tieneAcceso("fake-token")).thenReturn(true);
        when(productoService.obtenerProducto(1L)).thenReturn(producto);

        ResponseEntity<?> response = productoController.obtener(1L, "fake-token");
        Producto resultado = (Producto) response.getBody();
    ...
    @Test
    public void testExisteProductoConTokenValido() {
        when(authValidator.tokenValido("fake-token")).thenReturn(true);
        when(productoService.existeProducto(1L)).thenReturn(true);

        ResponseEntity<?> response = productoController.existe(1L, "fake-token");
    ...
    @Test
    public void testListarProductosSinRolPermitidoRetornaForbidden() {
        when(authValidator.tieneAcceso("fake-token")).thenReturn(false);
